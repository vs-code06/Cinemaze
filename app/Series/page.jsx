...
export default function Series() {
  ...
  useEffect(() => {
    const fetchGenres = async () => {
      const GENRE_API_URL = `https://api.themoviedb.org/3/genre/tv/list?api_key=${TMDB_API_KEY}&language=en-US`;
      try {
  ...
  // Filter series by search and genre
  const filteredSeries = series.filter(tv => {
    const matchesQuery = tv.name.toLowerCase().includes(searchQuery.toLowerCase());
    const matchesGenre =
      !selectedGenre || (tv.genre_ids && tv.genre_ids.includes(Number(selectedGenre)));
